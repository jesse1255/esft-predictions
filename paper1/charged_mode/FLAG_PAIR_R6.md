...
- **融合**：01 + 12 可以融合成連結 02 的雙繞環（荷 1 + 1 = 2，對應 α₁ + α₂），深 −132.2（結合 20.6%）。沿軸正面靠近要翻過 +31.9 的山（d ≈ 1）；**但這個山頂在三維裡有兩個下坡方向（兩環反向傾斜、從一側先合上的「拉鍊」模，§5.3），所以真正的能障嚴格小於 31.9**。三維格點計算（§5.4）：允許傾斜時，同樣的線 1 權重下能量是 −72（共軸 +26）；從分開側傾斜著接近，在 h = 0.3 的格點上幾乎看不到山（≲ 2，與格點誤差同量級，外推中）。「像核融合一樣要翻山」是軸對稱的假象。
...
- 從分開那一側傾斜著接近，在 h = 0.3 的格點上幾乎看不到山：重疊量 0.05 與 0.5 時 E_int 都已經 ≤ 0。這個量級（≲ 2）和格點誤差一樣大，**拉鍊路徑的能障究竟是 0 還是幾個單位，要等 h = 0.25、0.2 的外推**（計算中，§9）。
- 已經可以確定的：**「正面對撞 +31.9、像核融合一樣要翻山」這個說法是軸對稱造成的假象**。01 與 12 兩種漩渦只要能傾斜，就會從一側接觸、像拉鍊一樣合成 02 的雙繞環，需要的能量至多是共軸能障的一小部分。〔條件下的模型計算；κ₃ = 2、單一格點間距，外推進行中〕
...
3. **拉鍊能障的外推**：低重疊量掃描（O = 0.05–0.45）在 h = 0.3、0.25、0.2 重做，h² 外推；再用弦方法（`lattice_string.py`，不必選座標）確認過渡態。
