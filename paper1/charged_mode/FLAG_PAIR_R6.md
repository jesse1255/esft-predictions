...
- **整條軌跡沒有上坡**。前兩段只是去掉起點的插值誤差；之後兩環先稍微退開，**自己轉成幾乎交叉（60°–77°）、側向錯開約 3**，環緣碰在一起，再從接觸點拉上（第 13–14 段三線重疊只在拉鍊頭出現），最後變成一個 Q = 2 的雙繞環，放出約 132。
...
- **格點各向異性**：單一漩渦轉 15°、30°、45° 再鬆弛，能量比正放的低 0.29、0.22、0.18（h = 0.3）。所以在這個格點上「斜放」本身被偏愛 ≲ 0.3／環，上表 −0.3 到 −0.6 這種小數字**不能**解讀成吸引。
- **能障的上界**：沿這條明確的路徑，除了起點的插值誤差，能量從第 1 段起就 ≤ +0.62，加回各向異性（≲ 0.6）後 ≲ 1.2；再加上把兩環從無窮遠帶到 d ≈ 3.3 所需的能量（共軸 d = 3 是 +1.44，傾斜的更小）。所以**拉鍊／交叉路徑的能障 ≲ 1.5，不到共軸 31.9 的 5%**；在 h = 0.3 的解析度下與 0 相容。〔條件下的模型計算，單一格點間距；h = 0.25 的同一個實驗進行中〕
...
| α₁ + α₂ = α₁₂ | 連結 01 的漩渦 + 連結 12 的漩渦 → 連結 02 的雙繞環（荷 1 + 1 = 2，§4） | 只在軸對稱 L = (0, 1, 2) 扇區是數學事實。**三維裡不成立**：π₃(F₂) = ℤ 只有一個荷，連結標籤不守恆；三維格點上 01 + 12 融合成的是**連結 12** 的雙繞環（§5.4）。根的加法是那個扇區的纏繞數規則，不是動力學選擇規則 |
