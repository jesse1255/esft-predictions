...
- **轉動（雙螺旋）**：同向轉讓不同種類的漩渦斥力變小（d = 2 最多少三分之一），反向轉讓它變大，是一種旋向相關的力；越接近連結 02 的共振越強，但在不輻射的範圍內翻不成吸引。
...
- **結論**：同旋向的雙螺旋確實比較好靠近，但在允許的範圍（ω_A + ω_B < 1）內翻不成吸引。用轉動把兩個不同種類的漩渦綁住，這條路在 d = 2、3 走不通；它只會把融合的山坡削低一些（ω = 0.49 時 d = 2 的斥力少約三分之一）。〔條件下的模型計算〕
