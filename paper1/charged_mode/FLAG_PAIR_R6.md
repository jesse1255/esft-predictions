...
- **互扣（使用者提議的「史密斯環」，§5.5）**：01 環與 12 環互扣時，總荷是 **1 或 3**（視互扣方向），不是 2；交叉項的係數正是 SU(3) 的 Cartan 矩陣元 α₁·α₂ = −1（一般理論＋格點數值驗證，約 3%）。荷 1 的那一對會自己塌成**一個** α₁ + α₂ = α₁₂ 的環：根的加法在三維裡是互扣的規則。兩種環不能互相穿過；只在一點相接的史密斯組態正是荷區之間的分界。
...
![融合的下坡](figures/fusion_release_R6.png)
...
- **能障的上界**：沿這條明確的路徑，除了起點的插值誤差，能量從第 1 段起就 ≤ +0.62，加回格點釘扎（≈ 0.6）後 ≲ 1.2；再加上把兩環從無窮遠帶到 d ≈ 3.3 所需的能量（共軸 d = 3 是 +1.44，傾斜的更小）。所以**拉鍊／交叉路徑的能障 ≲ 1.5，不到共軸 31.9 的 5%**；在 h = 0.3 的解析度下與 0 相容。〔條件下的模型計算，單一格點間距；h = 0.25 的同一個實驗進行中〕
...
![史密斯圓圖與它在漩渦上的三維版本](figures/smith_chart_R6.png)
...
| 組態 | 荷 Q（格點） | 鬆弛後 |
|---|---:|---|
| 分開的一對（對照） | −1.94（＝2） | §5.4：交叉接觸後融合成 Q = 2 的環 |
...
- **兩種不同的環不能互相穿過**：一旦穿過，連結數改變 1，總荷就要改變 α₁·α₂ = −1，這在光滑的場裡不可能。所以「一點相接」的史密斯組態正好是三個荷區（1、2、3）之間的分界點；它的荷量出來是非整數，就是因為那一點是奇異的。
- 用使用者的話說：**一個連結 02 的環，拓撲上就等於一個 01 環和一個 12 環互扣在一起**——「同一個東西、裡面是兩樣東西」。但能量上單環低得多：互扣的一對不穩定，會自己塌成單環；反過來從單環「散發」出兩個互扣的環，至少要付約 400 的能量。
- 限制：單一格點間距；荷的估算有約 3% 的離散誤差；「Killing 形式」這一般敘述是依記憶，這裡只驗證了 α₁·α₂ = −1 這一個係數（互扣兩個方向都驗證了）。
...
- **大部分結果只在軸對稱、共軸、L = (0, 1, 2) 扇區。** 非軸對稱：融合鞍點的 k = 0–3 扇區已算（§5.3），融合路徑用三維格點重算（§5.4，單一間距 h = 0.3，外推中）；其他 l₃ 扇區、其他物體的非軸對稱穩定性（雙繞環 k = 1 已算：無負模）還沒有全部做完。§6 的轉動結果是共軸的，拉鍊路徑下轉動的效果沒有算。第三輪的門檻 κ₃* ≈ 0.919 屬於 L = (0, 1, 0) 扇區，本輪的門檻屬於 L = (0, 1, 2) 扇區；真正的穩定性要所有扇區都穩定。
...
3. **交叉路徑能障的解析**：同一個放開實驗在 h = 0.25（進行中）與 0.2 重做，各向異性隨 h² 縮小後看能障是否為 0；共軸對照（不傾斜的 d = 3 放開）；弦方法（`lattice_string.py`）確認過渡態。耦合不相等時（r、κ、m² 各連結不同），融合產物會選哪個連結。
